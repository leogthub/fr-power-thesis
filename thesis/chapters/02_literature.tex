...
clustering \citep{bunn2003} but are ill-suited to the extreme price spikes
...
who must justify their models to risk committees. \citet{zhao2023} confirm in a
recent comparative study that gradient boosted trees remain competitive with
neural network architectures on EPF tasks when sample sizes are moderate
($n < 100{,}000$ hourly observations), as in our setting.
...
\textit{wind power proxy} feature. \citet{lindberg2019} examine the interaction
between wind and hydro in Nordic markets, where weather plays a particularly
dominant role due to the high share of dispatchable hydro generation.

In the French market specifically, the thermosensitivity is unusually high by
European standards. \citet{rte2024} report that each degree Celsius below
