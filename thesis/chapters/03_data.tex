...
\begin{equation}
  \text{CDD}_t = \max(0,\ T_t - 23)
\end{equation}
where the 23\textdegree C threshold captures the onset of air-conditioning demand in France. CDD values are near zero for the vast majority of hours in the sample (mean $= 0.15$\textdegree C-hours), reflecting France's limited cooling load relative to its heating load.
