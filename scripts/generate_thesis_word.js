...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      p("Our central finding is that the marginal value of weather features is regime-dependent. In the stable 2024-25 period, weather features do not significantly improve forecast accuracy (Diebold-Mariano test p = 0.572). However, during the 2022 energy crisis, weather features become highly significant (DM = -13.27, p < 0.001), reducing MAE by 0.73 EUR/MWh. Both Random Forest models achieve MAE of approximately 16.9 EUR/MWh and R2 = 0.776 in the stable period, representing a 49% reduction in error over the naive benchmark. A day-ahead directional trading backtest confirms the economic value of these forecasts, with RF models generating net profits of approximately 136,700 EUR/MW over the test period (Sharpe ratio 19.5) with a maximum drawdown ten times smaller than the naive strategy."),
      sp(60, 60),
      ...
      p("This question is decomposed into four sub-questions:"),
      ...
      numbered("Does the addition of meteorological features produce a statistically significant improvement over a model built on market fundamentals alone — and is that contribution stable across market regimes?"),
      numbered("Does the marginal value of weather features differ between the stable 2024-25 period and the 2022 energy crisis? What structural mechanisms explain any difference?"),
      ...
      p("This thesis makes four contributions to the electricity price forecasting literature:"),
      ...
      bullet("Rigorous weather ablation with regime comparison: a four-model experiment (A/B/C/D) isolating the marginal contribution of weather features using Diebold-Mariano tests. Key finding: p = 0.572 in the stable 2024-25 regime; p < 0.001 (DM = -13.27) in the 2022 crisis. This supports the information redundancy hypothesis."),
      bullet("The 2022 energy crisis as a natural experiment: models retrained on 2018-2021 and tested on full-year 2022, revealing that extreme market regimes break the indirect weather channels and make raw meteorological variables directly informative."),
      ...
      h2("3.3 Descriptive Statistics"),
      p("The full dataset comprises 64,056 hourly observations after merging all sources and removing incomplete records. Table 0 below reports summary statistics for key variables."),
      sp(),
      makeTable([
        ["Variable", "Mean", "Std Dev", "Min", "Median", "Max", "Observations"],
        ["Price (EUR/MWh)",         "97.5",  "118.2", "-200.0", "70.2",  "3,086", "64,056"],
        ["TTF gas (EUR/MWh)",       "52.3",  "63.8",  "5.1",    "28.4",  "340.0", "64,056"],
        ["ARA coal (EUR/t)",        "127.4", "63.5",  "46.0",   "110.0", "423.0", "64,056"],
        ["Nuclear avail. ratio",    "0.72",  "0.12",  "0.31",   "0.73",  "0.97",  "64,056"],
        ["Load forecast (MW)",      "53,210","9,840", "31,800", "52,600","87,900","64,056"],
        ["HDD (°C)",                "3.21",  "4.88",  "0.00",   "0.32",  "21.5",  "64,056"],
        ["CDD (°C, 22°C threshold)","0.89",  "1.94",  "0.00",   "0.00",  "15.2",  "64,056"],
        ["Wind speed (m/s)",        "5.42",  "2.97",  "0.12",   "4.93",  "17.8",  "64,056"],
      ], [2200, 900, 900, 900, 1000, 900, 1200]),
      sp(60, 60),
      p("Table 0 — Summary statistics for key variables (n = 64,056 hourly observations, January 2018 - April 2025).", { italic: true, align: AlignmentType.CENTER }),
      sp(),
      p("Several distributional features are noteworthy. Price and generation variables (particularly gas-fired generation) are heavily right-skewed due to the 2022 energy crisis, which produced the extreme positive tail. Fuel prices — TTF gas and ARA coal — are leptokurtic, with excess kurtosis driven by the crisis period. The Cooling Degree Day (CDD) variable (computed with a 22 degrees C threshold) exhibits near-zero variability outside summer months, consistent with France's predominantly heating-driven thermosensitivity."),
      h2("3.4 Feature Engineering"),
      p("From the raw data sources, we construct 35 features organised into five categories."),
      h3("3.4.1 Calendar Features"),
      ...
      h3("3.4.3 Weather-Derived Features"),
      ...
      bullet("Cooling Degree Days (CDD): max(0, T - 22), where T is the 2m temperature. The 22 degrees C threshold captures cooling demand from air conditioning in warm months."),
      ...
      h3("3.4.4 Nuclear Availability Ratio"),
      ...
      h3("4.7.1 Performance Results"),
      sp(),
      makeTable([
        ["Model", "MAE (EUR/MWh)", "RMSE (EUR/MWh)", "sMAPE (%)", "R2", "Hit (%)"],
        ["A - Naive lag-168h",        "72.74", "111.11", "29.2", "0.419", "57.4"],
      ...
      sp(60, 60),
      p("Table 2b — Out-of-sample forecast performance, 2022 energy crisis validation (n = 8,760 hours).", { italic: true, align: AlignmentType.CENTER }),
      sp(),
      h3("4.7.2 Diebold-Mariano Tests — 2022"),
      sp(),
      makeTable([
        ["Comparison", "DM Stat.", "p-value", "Significance"],
        ["C vs B: does weather add value?",       "-13.27", "<0.001", "***"],
        ["C vs A: does RF-weather beat naive?",   "-6.48",  "<0.001", "***"],
        ["B vs A: does RF-no-weather beat naive?","-5.68",  "<0.001", "***"],
        ["D vs C: XGBoost vs RF-weather?",        "+26.57", "<0.001", "***"],
      ], [3200, 1200, 1200, 1500]),
      sp(60, 60),
      p("Table 2c — Diebold-Mariano tests for 2022 crisis period. *** p<0.001.", { italic: true, align: AlignmentType.CENTER }),
      sp(),
      p("Three key findings emerge from the 2022 validation. First, all models degrade sharply relative to the stable-period results — MAE roughly quadruples, reflecting the extreme price environment. Second, and critically, weather features become highly significant in 2022 (DM = -13.27, p < 0.001): Model C reduces MAE by 0.73 EUR/MWh over Model B, reversing the non-significant result from the stable period. Third, the XGBoost performance gap widens dramatically in the crisis regime (MAE 76.32 vs 66.55 EUR/MWh for RF), suggesting that boosting's sequential residual-fitting is especially vulnerable to the outlier prices of 2022."),
      h2("4.8 Explanation of the Central Result"),
      ...
      // ═══════════════════════════════════════════════
      h1("6. Discussion"),
      h2("6.1 The Central Finding: Weather Is Regime-Dependent"),
      p("The central empirical finding of this thesis has two parts. In the stable 2024-25 market regime, weather features do not significantly improve forecast accuracy (DM test p = 0.572). However, during the 2022 energy crisis, weather features become highly significant (DM = -13.27, p < 0.001), reducing MAE by 0.73 EUR/MWh over the fundamentals-only model. The marginal value of meteorological variables is therefore not a fixed property of the French market, but depends on the prevailing market regime."),
      h3("6.1.1 The Information Redundancy Hypothesis"),
      p("In stable market conditions, weather information is already embedded in other features through three channels:"),
      numbered("Load forecast as a temperature proxy: RTE's published load forecast already incorporates temperature forecasts. The correlation between HDD and the RTE load forecast exceeds r > 0.60. Including this feature implicitly conditions on temperature-adjusted demand, making the raw temperature variable redundant."),
      ...
      numbered("Nuclear availability dominance: the nuclear availability ratio explains a disproportionate share of price variance in France. With this feature included, residual variance is already low, leaving little room for weather variables to contribute independently."),
      h3("6.1.2 Why the Crisis Breaks Redundancy"),
      p("The 2022 energy crisis disrupted all three redundancy channels, making raw meteorological variables directly informative for the first time:"),
      numbered("Decoupling of TTF and local temperature: Russian gas curtailments drove TTF prices based on geopolitical supply factors, not European heating demand. TTF lost its weather signal — rising regardless of temperatures. The indirect weather channel through gas prices was severed, and raw temperature became directly informative about demand-driven price pressure."),
      numbered("Nuclear constraint amplification: with nuclear availability falling to approximately 40% (versus the typical 70-75%), there was no generation buffer to absorb demand shocks. A cold spell that would normally be absorbed by the flexible reserve forced dispatch of expensive gas and oil peaker units. This non-linear amplification made the temperature-to-price channel directly visible and statistically detectable."),
      numbered("Higher signal-to-noise ratio: the extreme price levels of 2022 produced larger absolute prediction errors across all models. Even a modest MAE improvement of 0.73 EUR/MWh (attributable to weather features) becomes statistically detectable by the DM test against this high-variance background."),
      h3("6.1.3 Implications"),
      p("These findings carry practical implications at three levels. For stable market conditions (the base case): fundamental models incorporating load forecast, fuel prices, nuclear availability, and cross-border flows are sufficient. The additional cost and operational complexity of integrating real-time weather forecasts is not statistically justified. For crisis market conditions: weather features provide significant incremental value and should be retained. We recommend a regime-switching approach triggered by observable indicators — specifically, nuclear availability ratio falling below 0.55 or TTF volatility exceeding historical norms — that activates the full weather feature set. For cross-market applicability: these results are specific to the French market. In Germany, where renewable penetration exceeds 50%, the direct supply-side impact of wind and solar on prices is unlikely to be fully captured by fuel prices or load forecasts, and weather features are expected to be significant even in stable regimes."),
      h2("6.2 XGBoost Underperformance"),
      p("XGBoost's sequential boosting mechanism appears more susceptible to the extreme price observations in the training set (2022 crisis prices above 1,000 EUR/MWh). Each boosting iteration gives elevated weight to poorly-fitted observations; in the presence of genuine outliers driven by extraordinary events, this can lead to overfitting to the crisis regime at the expense of normal-market performance. This pattern is confirmed by the 2022 validation experiment, where the XGBoost performance gap widens dramatically (MAE 76.32 vs 66.55 EUR/MWh for RF). Systematic hyperparameter optimisation using Bayesian search with a temporal validation strategy could close or reverse the performance gap in the stable period."),
      h2("6.3 Limitations"),
      ...
      numbered("Single primary test period: 12 months of one market regime. The 2022 crisis validation partially addresses this, but a rolling out-of-sample evaluation over multiple years would provide more robust assessment."),
      numbered("Missing EU ETS carbon allowances: European carbon allowance prices (EUA) are not included in the feature set despite their theoretical relevance to generation costs for gas and coal plants. This is acknowledged as a data limitation; EUA inclusion is left for future work."),
      ...
      bullet("Prioritise fundamental features (load forecast, fuel prices, nuclear availability, cross-border flows) in stable market regimes — the ablation study confirms weather adds no significant value. Retain weather feature capability for crisis regimes (low nuclear availability, high TTF volatility), where weather becomes significantly informative. A regime-switching trigger based on nuclear availability ratio and TTF volatility is recommended."),
